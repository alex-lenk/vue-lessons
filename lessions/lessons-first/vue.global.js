    ...
    ['COMPONENT_V_MODEL' /* COMPONENT_V_MODEL */]: {
      message: (comp) => {
        const configMsg = 'opt-in to ' +
                  `Vue 3 behavior on a per-component basis with \`compatConfig: { ${'COMPONENT_V_MODEL'}: false }\`.`
        if (comp.props &&
                  (isArray(comp.props)
  ...
  */
  ...
  function ensureValidVNode (vnodes) {
    return vnodes.some(child => {
      ...
      if (child.type === Fragment &&
              !ensureValidVNode(child.children)) { return false }
      return true
    })
      ? vnodes
      : null
  ...
  function parseComment (context) {
    ...
    if (!match) {
      ...
      let prevIndex = 1; let nestedIndex = 0
  ...
  function buildProps (node, context, props = node.props, ssr = false) {
    ...
    for (let i = 0; i < props.length; i++) {
      ...
      if (prop.type === 6 /* ATTRIBUTE */) {
        ...
        const valueNode = createSimpleExpression(value ? value.content : '', true, value ? value.loc : loc)
        if (name === 'ref') {
          hasRef = true
        }
